...
export default function SectionHeading({ index, eyebrow, title, intro }: Props) {
  return (
    <div className="mx-auto max-w-2xl text-center">
      ...
      <h2 className="mt-4 font-display text-3xl font-semibold tracking-tight text-text sm:text-4xl md:text-5xl">
        {title}
      </h2>
      {intro && <p className="mt-4 text-text-muted sm:text-lg">{intro}</p>}
    </div>
  )
}
